...
### 每个 Worker 都会持有一个线程对象 
 ...
 ```
### Worker 线程的运行
通过 ThreadFactory 创建线程对象时，会将 this 传入到 `ThreadFactory` 的 `newThread` 中，以下是 `ThreadFactory` 的默认实现类 `DefaultThreadFactory` 的 `newThread` 方法。
```java
...
此时会使用 CAS 将 ctl 中的 `Worker` 数量减一：
...
小结：`getTask` 决定了 `Worker` 是继续等待任务，还是因为线程池状态、Worker 数量、等待超时等原因退出。
